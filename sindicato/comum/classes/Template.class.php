...
class Template {
  ...
  function sdebug($var, $exit = false) { 
    ...
    echo "Template: A variável $varname não existe<br>";
...
//		throw new RuntimeException("var $varname não existe");
	...
	private function loadfile($varname, $filename) {
      ...
      echo "Template: O arquivo $filename não existe<br>";
    //throw new InvalidArgumentException("arquivo $filename não existe");
		...
		if (empty($str)) 
    echo "Template: O arquivo $filename está vazio<br>";
	...
	private function createBlocks(&$blocks) {
        ...
        echo "Template: Bloco duplicado: $chield<br>";        
	...
	private function setBlock($parent, $block) {
      ...
      echo "Template: O bloco $block está mal formado.<br>";            
	...
	private function subst($varname) {
		...
		foreach($this->instances as $var => $instance){
			foreach($this->properties[$var] as $properties){
              ...
              echo "Template: Não existe método na classe ".$class." para acessar ".$className."->".$property[$i]."<br>";
	...
	public function block($block, $append = true) {
      ...
      echo "Template: O bloco $block não existe.<br>";
