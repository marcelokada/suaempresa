<?php
  ...
  class Seguranca {
	...
	function registraLogin ($bd,$operador) {
      $sql = new Query($bd);
      $txt = "INSERT INTO TREDE_SEGTENTA(VNUMEROIPSEG,VNUMERCPFSEG,DDATATENTSEG,NNUMETENTSEG,VHORATENTSEG ) 
	  VALUES 
	  (:ip,:login,:datas,'1',:hora)";
      ...
      public function verificaSession($url_session){
          if (!headers_sent()){
              if(($url_session == null) or ($url_session == false)){
                  $pagina = 'comum/restrito.php';
                  header("Location: $pagina");
                  unset($_SESSION['aut_admin']);
                  unset($_SESSION['idSessao_admin']);
                  unset($_SESSION['usuaAdmin']);
              }
          }
      }
  
    public function verificaSessionNovo($url_session){
      if (!headers_sent()){
        if(($url_session == null) or ($url_session == false)){
          $pagina = 'comum/restrito.php';
          header("Location: $pagina");
