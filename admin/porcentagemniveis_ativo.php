<?php
  ...
  $tpl->addFile("CONTEUDO","porcentagemniveis_ativo.html");
  ...
  if ($_SESSION['nomeEmpresa'] == EMPRESA) { ///NOME DA EMPRESA
    ...
    if ($autenticado == TRUE) {
      ...
      $sql1 = new Query($bd);
      $txt1 = "SELECT NUMENIVEL,PORCENTAGEM,SEQ
      ...
      while (!$sql1->eof()) {
        ...
        $tpl->NIVEL = $sql1->result("NUMENIVEL");
      ...
      if (isset($_POST['alterar'])) {
        ...
        $sql2 = new Query($bd);
        $txt2 = "UPDATE TREDE_NIVEIS_ATIVOS SET PORCENTAGEM = :porc
                WHERE SEQ = :seq";
        $sql2->addParam(':porc',$porc);
        $sql2->addParam(':seq',$id);
        $sql2->executeSQL($txt2);
        
        header("location: porcentagemniveis_ativo.php?idSessao=".$id_sessao);
      }
