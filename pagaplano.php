...
$txt2 = "INSERT INTO TREDE_PAGAPLANO (NIDUPAGPLAN,
                      ...
                      CTIPOPGPLAN)
				VALUES
			('" . $idusu . "','" . $idplano . "','" . $valor . "','1','" . $tempo . "','" . date('Y-m-d H:i:s') . "','" . $mensa . "','" . $adesao . "','a','" . $oppag . "','p','" . $tipopags . "')";
...
$sql1 = new Query ($bd);
$txt1 = "SELECT LAST_INSERT_ID(SEQUPAGPLAN) SEQUPAGPLAN, NSEQPAGPLAN 
...
$sql81 = new Query($bd);
$txt81 = "SELECT NNUMENIVE ,NPORCNIVE FROM TREDE_NIVEL
WHERE  NIDUSNIVE IN (
SELECT NNUMEFILI FROM TREDE_NIVEL 
            WHERE NIDUSNIVE = :sequ1) ";
$sql81->addParam(':sequ1', $idusu);
$sql81->executeQuery($txt81);

$nivel_pat = $sql81->result("NNUMENIVE");

if ($nivel_pat == "") {
	$nivel_patrocinador = 0;
}
else {
	$nivel_patrocinador = $nivel_pat;
}

$sql4 = new Query ($bd);
$txt4 = "INSERT INTO TREDE_AFILIADOS_BONUS_ADESAO 
             (IDUSUABONUSAD, 
              NVLUSUBONUSAD, 
              NVALUSBONUSAD, 
              PORADEBONUSAD, 
...
$sql9 = new Query($bd);
$txt9 = "SELECT PORCENTAGEM FROM TREDE_NIVELPORCENT
            WHERE NIVEL = '" . $nivel_patrocinador . "'";
$sql9->executeQuery($txt9);

$porc_val = $sql9->result("PORCENTAGEM");
...
$valor_mensa_porc = $adesao * $porc_ativos / 100;
...
$res_valor_val = $sql6->result("VALORTOTAL");
...
if ($oppag == 'dotbank') {
	...
	$dados_identificador = array("customer_name"     => $nomeusua,
